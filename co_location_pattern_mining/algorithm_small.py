...
sys.stdout = open((cn.base_location+"output_small.txt"), "wt")
...
def create_table(pattern, mycursor, conn):
    query = '''CREATE TABLE IF NOT EXISTS "TiAI_Assignment1_InputData_Small"."'''+pattern+'''"
    ( '''
    for i in range(len(pattern)):
    ...
    ALTER TABLE IF EXISTS "TiAI_Assignment1_InputData_Small"."'''+pattern+'''"
    OWNER to postgres;'''
...
def create_index_all(pattern, mycursor, conn):
    for i in range(len(pattern)):
        ...
        ON "TiAI_Assignment1_InputData_Small"."small_'''+pattern[i]+'''"
        USING GIST(lat_long);'''
        mycursor.execute(query)
        conn.commit()
...
def drop_index_all(pattern, mycursor, conn):
    for i in range(len(pattern)):
        query = '''DROP INDEX IF EXISTS "TiAI_Assignment1_InputData_Small"."'''+pattern[i].lower()+'''_index";'''
        mycursor.execute(query)
        conn.commit()
...
def delete_init(pattern, mycursor, conn):
    ...
    if(len(pattern)==1):
        return
    query = '''DROP TABLE IF EXISTS "TiAI_Assignment1_InputData_Small"."'''+pattern+'''";'''
    mycursor.execute(query)
    conn.commit()
...
def delete_fin(mycursor, conn):
    for table in all_table_dict:
        query = '''DROP TABLE IF EXISTS "TiAI_Assignment1_InputData_Small"."'''+table+'''";'''
        mycursor.execute(query)
        conn.commit()
...
def truncate_table(mycursor, conn, pattern):
    query = '''TRUNCATE "TiAI_Assignment1_InputData_Small"."'''+pattern+'''";'''
    mycursor.execute(query)
    conn.commit()
...
def print_table(mycursor, conn, pattern):
    ...
    query = query + ''' FROM "TiAI_Assignment1_InputData_Small"."'''+pattern+'''";'''
    mycursor.execute(query)
...
def generate_size_2_queries(comb, mycursor, conn, threshold, h, X=0, Y=1):
    ...
    table_name_x = '"small_' + comb[X] + '"'
    table_name_y = '"small_' + comb[Y] + '"'
    query = ''' INSERT INTO "TiAI_Assignment1_InputData_Small"."'''+(comb)+'''" ('''+comb[X]+'''_id, '''+comb[Y]+'''_id)
                SELECT
                '''+comb[X]+'''.id AS '''+comb[X]+'''_id,
                '''+comb[Y]+'''.id AS '''+comb[Y]+'''_id
                FROM "TiAI_Assignment1_InputData_Small".''' + table_name_x+ ''' AS '''+comb[X]+'''
                JOIN "TiAI_Assignment1_InputData_Small".''' + table_name_y + ''' AS '''+comb[Y]+'''
                ON ST_DWithin('''+comb[X]+'''.lat_long, '''+comb[Y]+'''.lat_long, '''+ str(h) +''', true);'''
    mycursor.execute(query)
    add_query(comb, '''"TiAI_Assignment1_InputData_Small"."'''+(comb)+'''"''')
    conn.commit()
    print_table(mycursor, conn, comb)
    all_table_dict[comb] = '''"TiAI_Assignment1_InputData_Small"."'''+(comb)+'''"'''
    pi_val = participation_index(mycursor, comb)
    if(pi_val<threshold):
        truncate_table(mycursor, conn, comb)
        return 0 #means this pattern has failed
    
    table_dict[comb] = '''"TiAI_Assignment1_InputData_Small"."'''+(comb)+'''"'''
    return 1
...
def generate_size_k_queries(pattern, mycursor, conn, threshold=0.5):
    ...
    ########## INSERT clause ##########
    # INSERT INTO "TiAI_Assignment1_InputData_Small"."'''+(e_type_name[X] + e_type_name[Y])+'''" ('''+e_type_name[X]+'''_id, '''+e_type_name[Y]+'''_id)
    query = ''' INSERT INTO "TiAI_Assignment1_InputData_Small"."''' + pattern +'''" ('''
    for i in range(len(pattern)):
        j=(i+1)%(len(pattern))
    ...
    mycursor.execute(query)
    add_query(pattern, '''"TiAI_Assignment1_InputData_Small"."'''+pattern+'''"''')
    conn.commit()
    print_table(mycursor, conn, pattern)
    all_table_dict[pattern] = '''"TiAI_Assignment1_InputData_Small"."'''+(pattern)+'''"'''
    pi_val = participation_index(mycursor, pattern)
    if(pi_val<threshold):
        truncate_table(mycursor, conn, pattern)
        return 0 #means this pattern has failed
    table_dict[pattern] = '''"TiAI_Assignment1_InputData_Small"."'''+pattern+'''"'''
...
def participation_ratio(mycursor, X, pattern):
    if(len(pattern)==1):
        table_name_x = '"small_' + X + '"'
        table_name_y = '"small_' + X + '"'
    else:
        table_name_x = '"small_' + X + '"'
        table_name_y = '"' + pattern + '"'
    query = 'SELECT COUNT(*) FROM "TiAI_Assignment1_InputData_Small".' + table_name_x + ';'
    mycursor.execute(query)
    instances_tot_X = mycursor.fetchone()[0] # value of all instances of X
    query = ''' SELECT
                COUNT(DISTINCT '''+X+'''_id)
                FROM "TiAI_Assignment1_InputData_Small".''' + table_name_y+ ''';'''
    mycursor.execute(query)
...
def master_func(master_pattern, threshold, h):
    ...
    print("\nEligible colocation patterns: ", end ="")
